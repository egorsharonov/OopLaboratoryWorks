...
template<typename K, class hash>
HashTable<K, hash>::HashTable(const HashTable& other)
{
...
template<typename K, class hash>
HashTable<K, hash>::HashTable(int c)
{
    int size = getPrime(c);
    this->capacity = size;
    table = new list<ValueType>[capacity];
...
template<typename K, class hash>
void HashTable<K, hash>::operator<<(K key)
{
    uint64_t cache = hash{}(key);
    int index = cache % capacity;
    if (find(key)) {
        for (auto& x : table[index])
            if (x.key_ == key) ++x.data_;
    }
    else{
        ++size;
        ...
        keys.push_back(key);
        auto pos = --keys.end();
        table[index].push_back(ValueType(key, 1, cache, pos));
    }
}

template<typename K, class hash>
void HashTable<K, hash>::deleteItem(K key)
{
    if (not find(key)) {
        return;
    }
...
template<typename K, class hash>
void HashTable<K, hash>::print() const{
    for( auto key : keys){
        auto value = (*this)[key];
        std::cout  << key << "  :  " << value << "\n";
    }
...
template<typename K, class hash>
int HashTable<K, hash>::countRepeats(int num) const
{
    int count = 0;
    for(auto& key : keys){
        count += ((*this)[key]>=num);
...
template<typename K, class hash>
bool HashTable<K, hash>::find(K key) const
{
...
template<typename K, class hash>
void HashTable<K, hash>::write(const string& path) const
{
...
template<typename K, class hash>
void HashTable<K, hash>::read(const string& path)
{
    clear();
...
template<typename K, class hash>
int HashTable<K, hash>::operator[](K key) const
{
    uint64_t cache = hash{}(key);
    int index = cache % capacity;
    for (auto& x : table[index]) {
...
template<typename K, class hash>
bool HashTable<K, hash>::operator==(const HashTable& other) const
{
